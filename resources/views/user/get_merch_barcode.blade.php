  ...
  <div class="row">
    <div class="col-lg-4 text-center">
        ...
        <div class="card_portofolio">
            @php $en1 = base64_encode($data->id_ordermerch)@endphp
            @if($data->used_ordermerch == 1)
          ...
          <center>
            ...
            @else
          <img style="height: 300px; margin-top: -25px;" src="data:image/png;base64,{{DNS2D::getBarcodePNG('https://biggest.telesandifestival.com/checked/'.base64_encode($en1), 'QRCODE')}}" alt="barcode" />
            @endif
          </div>
    </div>
    ...
    <div class="col-lg-8"><br>
      <div class="table-responsive">
        <table class="table bg-white">
          <tbody>
            <tr>
              <th style="font-size: 1.23rem; width: 30%; " scope="col">Nama Produk</th>
              ...
              <td style="font-size: 1.33rem">{{ $data->nama_merch }}@if($data->kategori_merch == "BAJU") {{ ' - '.$data->ukuran_ordermerch }} @endif</td>
            </tr>
            <tr>
              <th style="font-size: 1.23rem; width: 30%; " scope="col">Harga</th>
    								@php 
    								$hiya = $data->harga_merch;
    								if($data->kategori_merch == "BAJU" && $data->ukuran_ordermerch == 'S' OR 'M' OR 'L'){
    								$hiya = $data->harga_merch;
    								}
    								if($data->kategori_merch == "BAJU" && $data->ukuran_ordermerch == 'XL'){
    								$hiya = $data->harga_merch + 5000;
    								}
    								if($data->kategori_merch == "BAJU" && $data->ukuran_ordermerch == 'XXL'){
    								$hiya = $data->harga_merch + 10000;
    								}
    								 @endphp
              <td style="font-size: 1.33rem;">Rp. {{number_format($hiya,2)}}</td>
            </tr>
            <tr>
              <th style="font-size: 1.23rem; width: 30%; " scope="col">Jumlah</th>
              <td style="font-size: 1.33rem;">{{$data->jumlah_ordermerch}}</td>
            </tr>
            <tr>
              <th style="font-size: 1.23rem; width: 30%; " scope="col">Subtotal</th>
              <td style="font-size: 1.33rem;">Rp. {{number_format($hiya * $data->jumlah_ordermerch + $data2->kode_ordermerch,2)}}</td>
            </tr>
            <tr>
